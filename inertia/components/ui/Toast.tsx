...
/**
 * ToastContainer - Automatically displays toasts from Inertia flash messages
 */
export function ToastContainer() {
  ...
  useEffect(() => {
    const newToasts: Array<{ id: string; message: string; type: 'success' | 'error' | 'info' }> = []

    if (flash?.success) {
      newToasts.push({ id: `success-${Date.now()}`, message: flash.success, type: 'success' })
    }
    if (flash?.error) {
      newToasts.push({ id: `error-${Date.now()}`, message: flash.error, type: 'error' })
    }
    if (flash?.info) {
      newToasts.push({ id: `info-${Date.now()}`, message: flash.info, type: 'info' })
    }
