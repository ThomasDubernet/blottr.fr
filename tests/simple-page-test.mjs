...
async function testPage(browser, pageInfo, viewport) {
  ...
  page.on('requestfailed', (request) => {
    results.errors.push(
      `[${viewport.name}] Request failed: ${request.url()} - ${request.failure()?.errorText || 'Unknown error'}`
    )
  })
  ...
  try {
    ...
    const screenshotPath = path.join(
      screenshotDir,
      `${pageInfo.name.toLowerCase().replace(' ', '-')}-${viewport.name}.png`
    )
...
async function testBasicFunctionality(page, pageInfo, viewport, results) {
  try {
    ...
    const hasReact = await page.evaluate(() => {
      return (
        window.React !== undefined || document.querySelector('[data-reactroot], #root') !== null
      )
    })
...
async function testDesignSystemPage(page, viewport, results) {
  try {
    ...
    const showcaseElements = await page.$$(
      '[class*="artist"], [class*="tattoo"], [class*="gallery"]'
    )
...
async function runTests() {
    ...
    for (const pageInfo of pages) {
      ...
      for (const viewport of viewports) {
        ...
        if (result.status === 'success') {
          ...
          console.log(
            `    ❌ FAIL (${result.errors.length} errors, ${result.consoleErrors.length} console errors)`
          )
  ...
  const report = {
    ...
    summary: {
      ...
      responsiveIssues: allResults.filter((r) => r.errors.some((err) => err.includes('scroll')))
        .length,
    },
  }
