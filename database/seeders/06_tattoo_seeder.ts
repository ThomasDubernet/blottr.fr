...
export default class extends BaseSeeder {
  async run() {
    ...
    const emma = artists.find((a) => a.slug === 'emma-laurent-paris-minimalist')
    const nina = artists.find((a) => a.slug === 'nina-rodriguez-paris-independent')
    ...
    const tattoos = [
      ...
      },
      {
        ...
        description: 'Composition triangulaire abstraite avec jeu de lignes et espaces négatifs.',
        isFlash: true,
        price: 280.0,
      ...
      },
      {
        ...
        description: 'Phases de lune en fine line. Délicatesse et poésie dans la simplicité.',
        isFlash: true,
        price: 95.0,
      ...
      },
      {
        ...
        description: 'Constellation personnalisée avec points reliés. Minimalisme astronomique.',
        isFlash: false,
        price: null,
      ...
      // Additional works from other artists
      {
        ...
        description: 'Motif tribal moderne en blackwork. Lignes épurées et contraste saisissant.',
        isFlash: true,
        price: 200.0,
    ...
    if (createdTattoos.length > 0) {
      ...
      if (realismTag && noirGrisTag && animauxTag) {
        ...
        await createdTattoos[1]
          .related('tags')
          .attach([realismTag.id, noirGrisTag.id, animauxTag.id].filter(Boolean))
      }
      ...
      if (traditionalTag && fleursTag && couleurTag) {
        await createdTattoos[2]
          .related('tags')
          .attach([traditionalTag.id, fleursTag.id, couleurTag.id].filter(Boolean))
        await createdTattoos[3]
          .related('tags')
          .attach([traditionalTag.id, animauxTag?.id, couleurTag.id].filter(Boolean))
      }
      ...
      if (minimalistTag) {
        ...
        await createdTattoos[7].related('tags').attach([minimalistTag.id].filter(Boolean))
        await createdTattoos[8].related('tags').attach([minimalistTag.id].filter(Boolean))
      }

      // Other artists
      if (blackworkTag && watercolorTag) {
        await createdTattoos[10].related('tags').attach([blackworkTag.id].filter(Boolean))
        await createdTattoos[11]
          .related('tags')
          .attach([watercolorTag.id, couleurTag?.id, animauxTag?.id].filter(Boolean))
      }
    }
