...
export default class extends BaseSeeder {
  async run() {
    ...
    const inkMasters = salons.find(
      (salon) => salon.slug === "ink-masters-paris",
    );
    const blackRose = salons.find(
      (salon) => salon.slug === "black-rose-tattoo-lyon",
    );
    const electricOctopus = salons.find(
      (salon) => salon.slug === "electric-octopus-marseille",
    );
    const minimalInk = salons.find(
      (salon) => salon.slug === "minimal-ink-studio-paris",
    );
    ...
    const artists = [
      ...
      // Instagram scraped artists (in various onboarding stages)
      {
        ...
        contactCount: 3,
        gptBio:
          "Artiste émergent spécialisé dans le blackwork et les motifs tribaux modernes.",
        gptStyles: {
      ...
      },
      {
        ...
        contactCount: 8,
        gptBio:
          "Peintre de la peau, Léa apporte la magie de l'aquarelle au monde du tatouage.",
        gptStyles: {
