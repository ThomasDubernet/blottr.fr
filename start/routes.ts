...
router
  .group(() => {
    // Artists routes - Public API
    router.get('/artists', [ArtistsController, 'index']).as('api.artists.index')

    // Contact Inquiry routes - temporarily without middleware for testing
    router
      .post('/contact-inquiries', [ContactInquiriesController, 'store'])
      .as('api.contact-inquiries.store')
    router
      .post('/contact-inquiries/quick', [ContactInquiriesController, 'storeQuick'])
      .as('api.contact-inquiries.quick')
    router
      .get('/contact-inquiries/:id', [ContactInquiriesController, 'show'])
      .as('api.contact-inquiries.show')
    router
      .get('/health/contact-inquiries', [ContactInquiriesController, 'health'])
      .as('api.health.contact-inquiries')
  })
  .prefix('/api')

// Artist routes
router
  .get('/artistes/:slug', async ({ params, inertia }) => {
